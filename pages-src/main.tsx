...
function Main() {
  ...
  async function connect() {
    ...
    try {
      ...
      setReady(true);
      setMessage("已連接私人資產資料庫");
    } catch (e) { setMessage(e instanceof Error ? e.message : "Google 連接失敗，請重試。"); }
    finally { setBusy(false); }
  }
  return <>
    <section className={ready ? "m-3 rounded-xl border border-slate-200 bg-white p-3 text-sm sm:mx-6" : "mx-auto mt-12 w-[calc(100%-2rem)] max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:mt-20 sm:p-8"}>
      {!ready && <>
    ...
    {ready && <Home baseline={settings.baseline} />}
  </>;
}
